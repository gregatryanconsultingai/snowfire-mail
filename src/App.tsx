...
type PriorityLevel = 'top'|'high'|'medium'|'low'
type MessagePriorities = Record<string,PriorityLevel>
type SenderPriorityRules = Record<string,PriorityLevel>
type DashboardMessage = {message:Message;meta:MessageMeta;priority?:PriorityLevel}
type RoutingFolderNode = {name:string;fullName:string;children:RoutingFolderNode[];rules:string[];count:number}
type InlineFolderDraft = {parentName:string;afterName?:string;name:string;error:string}
type SidebarFolderEntry = GmailLabel|{id:'__inline-folder-draft__';draft:true}
...
const PRIORITY_LEVELS:PriorityLevel[]=['top','high','medium','low']
const PRIORITY_META:Record<PriorityLevel,{label:string;color:string;description:string}>={top:{label:'Top',color:'#ff4f8b',description:'Needs immediate attention'},high:{label:'High',color:'#ff984f',description:'Important and time-sensitive'},medium:{label:'Medium',color:'#ad7cff',description:'Worth handling soon'},low:{label:'Low',color:'#50cbd4',description:'Keep on the radar'}}
const priorityRank=(priority?:PriorityLevel)=>priority?PRIORITY_LEVELS.indexOf(priority):Number.MAX_SAFE_INTEGER
...
function PriorityBadge({level,compact=false}:{level:PriorityLevel;compact?:boolean}) {
  const meta=PRIORITY_META[level]
  return <span className={`priority-badge priority-${level}${compact?' compact':''}`} style={{'--priority-color':meta.color} as CSSProperties}><i/>{meta.label}</span>
}

function PriorityRuleList({rules,busy,onRemove}:{rules:SenderPriorityRules;busy:boolean;onRemove:(email:string)=>void}) {
  const [expanded,setExpanded]=useState<Set<PriorityLevel>>(()=>new Set(['top','high']))
  const toggle=(level:PriorityLevel)=>setExpanded(current=>{const next=new Set(current);if(next.has(level))next.delete(level);else next.add(level);return next})
  const groups=PRIORITY_LEVELS.map(level=>({level,emails:Object.entries(rules).filter(([,priority])=>priority===level).map(([email])=>email).sort((a,b)=>a.localeCompare(b))}))
  return <div className="priority-rule-list" role="tree" aria-label="Priority rules by level">{groups.map(({level,emails})=>{const open=expanded.has(level),meta=PRIORITY_META[level];return <div className={`priority-rule-group priority-${level}`} style={{'--priority-color':meta.color} as CSSProperties} role="treeitem" aria-expanded={open} key={level}><button type="button" className="priority-rule-heading" onClick={()=>toggle(level)}><ChevronDown className={open?'open':''} size={12}/><PriorityBadge level={level}/><span>{meta.description}</span><i>{emails.length}</i></button>{open&&<div role="group" className="priority-rule-children">{emails.length?emails.map(email=><div className="priority-rule-row" role="treeitem" key={email}><span><Mail size={13}/></span><div><b>{email}</b><small>Exact sender match</small></div><button type="button" aria-label={`Remove priority rule for ${email}`} title="Remove rule" disabled={busy} onClick={()=>onRemove(email)}><X size={14}/></button></div>):<p>No {meta.label.toLowerCase()} priority rules.</p>}</div>}</div>})}</div>
}

function InboxDashboard({coworkers,clients,prioritized,unread,onOpen}:{coworkers:DashboardMessage[];clients:DashboardMessage[];prioritized:DashboardMessage[];unread:number;onOpen:(id:string)=>void}) {
  const renderLane=(title:string,eyebrow:string,items:DashboardMessage[],Icon:LucideIcon,tone:string,empty:string)=><section className={`inbox-lane ${tone}`}><header><span><Icon size={17}/></span><div><small>{eyebrow}</small><h2>{title}</h2></div><i>{items.length}</i></header><div className="inbox-lane-items">{items.slice(0,5).map(({message,priority})=><button key={message.id} onClick={()=>onOpen(message.id)}><span className="dashboard-avatar" style={{background:message.color}}>{message.initials}</span><span className="dashboard-message-copy"><span><b>{message.sender}</b><time>{message.time}</time></span><strong>{message.subject}</strong><small>{message.preview}</small></span>{priority&&<PriorityBadge level={priority} compact/>}</button>)}{!items.length&&<div className="inbox-lane-empty"><Icon size={19}/><span>{empty}</span></div>}</div></section>
  return <div className="inbox-dashboard"><header className="inbox-dashboard-hero"><div><span>INBOX / PRIORITY PULSE</span><h1>Your attention, beautifully sorted.</h1><p>Coworkers, clients, and the messages that matter most—without digging.</p></div><div className="inbox-dashboard-stats"><span><b>{unread}</b><small>Unread</small></span><span><b>{prioritized.length}</b><small>Prioritized</small></span><span><b>{coworkers.length+clients.length}</b><small>People</small></span></div></header><div className="inbox-dashboard-grid">{renderLane('Coworkers','YOUR TEAM',coworkers,UsersRound,'coworkers','No coworker mail in view.')}{renderLane('Clients','RELATIONSHIPS',clients,BriefcaseBusiness,'clients','No client mail in view.')}{renderLane('Priority','FOCUS QUEUE',prioritized,Flame,'priority','Mark an email to build your focus queue.')}</div><div className="all-inbox-divider"><span>All inbox</span><i/></div></div>
}
...
function FolderDeleteConfirmation({folder,folders,busy,error,standalone=false,onCancel,onDelete}:{folder:GmailLabel;folders:GmailLabel[];busy:boolean;error:string;standalone?:boolean;onCancel:()=>void;onDelete:()=>void}) {
  const hasChildren=folders.some(label=>label.name.startsWith(`${folder.name}/`))
  return <div className={`label-delete-confirm${standalone?' standalone':''}`} onMouseDown={()=>{if(standalone&&!busy)onCancel()}}><div role="dialog" aria-modal="true" aria-labelledby="folder-delete-title" onMouseDown={event=>event.stopPropagation()}><span><Trash2 size={18}/></span><h3 id="folder-delete-title">Delete “{folderName(folder.name)}”?</h3><p>This removes the folder{hasChildren?' and its subfolders':''} from Gmail. Emails inside will be archived, not deleted.</p>{error&&<div className="folder-delete-error">{error}</div>}<footer><button autoFocus disabled={busy} onClick={onCancel}>Cancel</button><button className="delete-label-button" disabled={busy} onClick={onDelete}>{busy?'Deleting…':'Delete folder'}</button></footer></div></div>
}
...
function App() {
  ...
  const [messagePriorities, setMessagePriorities] = useState<MessagePriorities>({})
  const [senderPriorityRules, setSenderPriorityRules] = useState<SenderPriorityRules>({})
  ...
  const [inlineFolderDraft, setInlineFolderDraft] = useState<InlineFolderDraft|null>(null)
  ...
  const [priorityRuleEmail, setPriorityRuleEmail] = useState('')
  const [priorityRuleLevel, setPriorityRuleLevel] = useState<PriorityLevel>('high')
  const [priorityRuleError, setPriorityRuleError] = useState('')
  ...
  const [inboxOverviewMessages, setInboxOverviewMessages] = useState<Message[]>([])
  ...
  const inlineFolderCreating = useRef(false)
  ...
  const sidebarFolderEntries = useMemo<SidebarFolderEntry[]>(()=>{const entries:SidebarFolderEntry[]=[...visibleFolderLabels];if(!inlineFolderDraft)return entries;const draft:SidebarFolderEntry={id:'__inline-folder-draft__',draft:true};if(!inlineFolderDraft.afterName){entries.unshift(draft);return entries}let index=entries.findIndex(entry=>!('draft' in entry)&&entry.name===inlineFolderDraft.afterName);if(index>=0&&inlineFolderDraft.parentName!==inlineFolderDraft.afterName)while(index+1<entries.length){const next=entries[index+1];if('draft' in next||!folderContains(inlineFolderDraft.afterName,next.name))break;index++}entries.splice(index<0?0:index+1,0,draft);return entries},[visibleFolderLabels,inlineFolderDraft])
  ...
  const effectivePriority = (message:Message):PriorityLevel|undefined => messagePriorities[message.id]||senderPriorityRules[normalizeSenderEmail(message.email)]
  const inboxDashboard=useMemo(()=>{
    const source=connected&&inboxOverviewMessages.length?inboxOverviewMessages:messages
    const items=source.map(message=>({message,meta:messageMeta(message,gmailLabels,labelIcons,senderLabels),priority:messagePriorities[message.id]||senderPriorityRules[normalizeSenderEmail(message.email)]} satisfies DashboardMessage))
    const newest=(a:DashboardMessage,b:DashboardMessage)=>(b.message.receivedAt??0)-(a.message.receivedAt??0)
    const routeFor=(message:Message)=>(senderFolderRules[normalizeSenderEmail(message.email)]||'').toLocaleLowerCase()
    return {coworkers:items.filter(item=>item.meta.tone==='internal'||/(^|\/)coworkers?(\/|$)/.test(routeFor(item.message))).sort(newest),clients:items.filter(item=>item.meta.tone==='customer'||/^folders\/clients?(\/|$)/.test(routeFor(item.message))).sort(newest),prioritized:items.filter(item=>item.priority).sort((a,b)=>priorityRank(a.priority)-priorityRank(b.priority)||newest(a,b))}
  },[connected,inboxOverviewMessages,messages,gmailLabels,labelIcons,senderLabels,senderFolderRules,messagePriorities,senderPriorityRules])
  const showInboxDashboard=folder==='Inbox'&&!selected&&!query.trim()
  ...
  const sync = async () => { if(!window.snowfire||!connected)return; setSyncing(true);setSelectedIds(new Set());try{await Promise.all([loadMailbox(query.trim()?`in:anywhere ${query.trim()}`:currentFolderQuery(),folder!=='Inbox'||Boolean(selected)),refreshMailboxIndicators(true)])}catch(e){flash(e instanceof Error?e.message:'Sync failed')}finally{setSyncing(false)} }
  ...
  useEffect(()=>{window.snowfire?.metadata.messagePriorities().then(priorities=>setMessagePriorities(priorities||{})).catch(()=>{})},[])
  useEffect(()=>{window.snowfire?.metadata.senderPriorityRules().then(rules=>setSenderPriorityRules(rules||{})).catch(()=>{})},[])
  ...
  useEffect(()=>{if(!connected||folder!=='Inbox'||!window.snowfire){if(!connected)setInboxOverviewMessages([]);return}let live=true;const timer=window.setTimeout(()=>{window.snowfire!.gmail.list('in:anywhere -in:sent -in:drafts -in:trash -in:spam',settings.remoteImages).then(mail=>{if(live)setInboxOverviewMessages(mail||[])}).catch(()=>{})},300);return()=>{live=false;window.clearTimeout(timer)}},[folder,connected,settings.remoteImages])
  ...
  const openDashboardMessage = (id:string) => {const current=messages.find(message=>message.id===id)||inboxOverviewMessages.find(message=>message.id===id);if(!current)return;setSelected(id);setMessages(items=>items.some(message=>message.id===id)?items.map(message=>message.id===id?{...message,unread:false}:message):[{...current,unread:false},...items]);setInboxOverviewMessages(items=>items.map(message=>message.id===id?{...message,unread:false}:message));if(connected&&current.unread)window.snowfire?.gmail.modify(id,[],['UNREAD']).then(()=>refreshMailboxIndicators(true)).catch(()=>flash('Could not mark that message as read'));setResponseMode(null);setSnoozeMenuOpen(false);setSenderDetailsOpen(false)}
  const setPriorityForMessage = async (message:Message,level:PriorityLevel|undefined) => {const previous=messagePriorities,next={...previous};if(level)next[message.id]=level;else delete next[message.id];setMessagePriorities(next);try{const saved=await window.snowfire?.metadata.setMessagePriority(message.id,level||'');if(saved)setMessagePriorities(saved);flash(level?`${PRIORITY_META[level].label} priority · ${message.subject}`:'Priority cleared')}catch(e){setMessagePriorities(previous);flash(e instanceof Error?e.message:'Could not save priority')}}
  ...
  const openFolder = async (name:string, labelId?:string, messageId?:string) => {
    ...
    try { const mail=await loadMailbox(labelId?folderLabelQuery(labelId):(folderQuery[name]||'in:inbox'),name!=='Inbox'||Boolean(messageId));const target=messageId?mail.find(message=>message.id===messageId):undefined;if(target){setSelected(target.id);if(target.unread){setMessages(items=>items.map(message=>message.id===target.id?{...message,unread:false}:message));window.snowfire.gmail.modify(target.id,[],['UNREAD']).then(()=>refreshMailboxIndicators(true)).catch(()=>{})}} }
  ...
  const beginInlineFolderCreate = (parentName='',afterName?:string) => {
    const parent=parentName===FOLDER_ROOT?'':parentName
    setFolderContextMenu(null);setFoldersOpen(true);setFolderError('');setInlineFolderDraft({parentName:parent,afterName,name:'',error:''})
    if(parent)setCollapsedFolders(current=>{const next=new Set(current);let ancestor=parent;while(ancestor!==FOLDER_ROOT){next.delete(ancestor);ancestor=folderParent(ancestor)}try{localStorage.setItem(COLLAPSED_FOLDERS_KEY,JSON.stringify([...next]))}catch{}return next})
    window.requestAnimationFrame(()=>window.requestAnimationFrame(()=>inlineFolderInputRef.current?.focus()))
  }
  const createInlineFolder = async () => {
    if(!inlineFolderDraft||folderBusy||inlineFolderCreating.current)return
    const leaf=inlineFolderDraft.name.trim().replace(/^\/+|\/+$/g,'').replace(/\/+/g,' ')
    if(!leaf){setInlineFolderDraft(null);return}
    const name=inlineFolderDraft.parentName?`${inlineFolderDraft.parentName}/${leaf}`:`${FOLDER_ROOT}/${leaf}`
    if(folderLabels.some(label=>label.name.toLocaleLowerCase()===name.toLocaleLowerCase())){const error=`A folder named ${leaf} already exists here`;setInlineFolderDraft(current=>current?{...current,error}:current);flash(error);window.requestAnimationFrame(()=>inlineFolderInputRef.current?.select());return}
    if(!window.snowfire){flash('Connect Gmail before creating a folder');return}
    inlineFolderCreating.current=true;setFolderBusy(true)
    try{
      const created=await window.snowfire.gmail.createLabel(name) as GmailLabel
      setGmailLabels(labels=>labels.some(label=>label.id===created.id||label.name.toLocaleLowerCase()===created.name.toLocaleLowerCase())?labels:[...labels,created])
      setFolderOrder(current=>{const next=[...current.filter(item=>item!==created.name),created.name];window.snowfire?.metadata.setFolderOrder(next).catch(()=>{});return next})
      setInlineFolderDraft(null);flash(`Created ${folderName(created.name)}`)
    }catch(e){const error=e instanceof Error?e.message:'Could not create the folder';setInlineFolderDraft(current=>current?{...current,error}:current);flash(error);window.requestAnimationFrame(()=>inlineFolderInputRef.current?.focus())}
    finally{inlineFolderCreating.current=false;setFolderBusy(false)}
  }
  const updateRemoteImageSetting = async (remoteImages:boolean) => {setSettings({remoteImages});try{await window.snowfire?.settings.update({remoteImages});if(connected){setMailboxLoading(true);const mail=await window.snowfire!.gmail.list(currentFolderQuery(),remoteImages);setMessages(mail);setSelected(folder==='Inbox'?null:mail[0]?.id||null)}}catch(e){flash(e instanceof Error?e.message:'Could not save settings')}finally{setMailboxLoading(false)}}
  ...
  const disconnectGmail = async () => {await window.snowfire?.gmail.disconnect();setConnected(false);setAccount('');setSettingsOpen(false);setMessages(initialMessages);setGmailLabels(initialLabels);setRecipientContacts([]);setSelected(null);flash('Gmail disconnected')}
  ...
  const createSenderPriorityRule = async () => {const email=normalizeSenderEmail(priorityRuleEmail);if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){setPriorityRuleError('Enter a valid sender email address.');return}setRuleBusy(true);setPriorityRuleError('');try{const saved=await window.snowfire?.metadata.setSenderPriorityRule(email,priorityRuleLevel)||{...senderPriorityRules,[email]:priorityRuleLevel};setSenderPriorityRules(saved);setPriorityRuleEmail('');flash(`${PRIORITY_META[priorityRuleLevel].label} priority rule saved for ${email}`)}catch(e){setPriorityRuleError(e instanceof Error?e.message:'Could not save the priority rule')}finally{setRuleBusy(false)}}
  const removeSenderPriorityRule = async (email:string) => {setRuleBusy(true);setPriorityRuleError('');try{const next=await window.snowfire?.metadata.removeSenderPriorityRule(email)||Object.fromEntries(Object.entries(senderPriorityRules).filter(([sender])=>sender!==email)) as SenderPriorityRules;setSenderPriorityRules(next);flash(`Removed priority rule for ${email}`)}catch(e){setPriorityRuleError(e instanceof Error?e.message:'Could not remove the priority rule')}finally{setRuleBusy(false)}}
  const openPriorityRuleForSender = (message:Message) => {const email=normalizeSenderEmail(message.email);setPriorityRuleEmail(email);setPriorityRuleLevel(senderPriorityRules[email]||effectivePriority(message)||'high');setPriorityRuleError('');setRoutingManager(true);setContextMenu(null)}
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className={`content ${folder==='Calendar'?'calendar-content':folder==='Folders'?'folder-dashboard-content':'with-day-column'}`}>
        ...
        <section className={`${selected?'mail-list has-reader':'mail-list'}${showInboxDashboard?' inbox-dashboard-mode':''}`}>
          ...
          <div className="message-list">{mailboxLoading?<div className="mailbox-state"><RefreshCw className="spin-icon" size={21}/><b>Loading {folder}</b><span>Syncing with Gmail</span></div>:visible.length===0?<div className="mailbox-state empty"><Inbox size={24}/><b>Nothing in {folder}</b><span>This mailbox is clear.</span></div>:visible.map(m=>{const meta=messageMeta(m,gmailLabels,labelIcons,senderLabels),priority=effectivePriority(m);return <article key={m.id} style={{'--meta-color':meta.color} as CSSProperties} draggable onDragStart={event=>{const ids=selectedIds.has(m.id)?Array.from(selectedIds):[m.id];setDraggingIds(ids);event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('application/x-snowfire-message-ids',ids.join(','));event.dataTransfer.setData('application/x-snowfire-email-id',m.id);event.dataTransfer.setData('text/plain',m.subject)}} onDragEnd={()=>{setDraggingIds([]);setDropLabelId(null)}} onClick={()=>selectMessage(m.id)} onContextMenu={event=>{event.preventDefault();setSelected(m.id);setShowContextLabels(false);setContextMenu({x:Math.min(event.clientX,window.innerWidth-224),y:Math.min(event.clientY,window.innerHeight-286),message:m})}} className={`${selected===m.id?'selected ':''}${selectedIds.has(m.id)?'bulk-selected ':''}${draggingIds.includes(m.id)?'dragging ':''}${m.unread?'unread':''}`}>
            ...
            <div className="message-copy">
              <div className={`message-meta-overline ${meta.tone}`} style={{color:meta.color}}><MetaIcon meta={meta}/><span>{meta.label}</span>{priority&&<PriorityBadge level={priority} compact/>}</div>
    ...
    {folderManager&&<div className="label-modal-backdrop" onMouseDown={()=>{if(!folderBusy){setFolderManager(false);setConfirmDeleteFolder(null)}}}><section className="label-manager folder-manager" role="dialog" aria-modal="true" aria-labelledby="folder-manager-title" onMouseDown={event=>event.stopPropagation()}>
      ...
      {confirmDeleteFolder&&<FolderDeleteConfirmation folder={confirmDeleteFolder} folders={folderLabels} busy={folderBusy} error={folderError} onCancel={()=>setConfirmDeleteFolder(null)} onDelete={removeFolder}/>}
    </section></div>}
    {confirmDeleteFolder&&!folderManager&&<FolderDeleteConfirmation standalone folder={confirmDeleteFolder} folders={folderLabels} busy={folderBusy} error={folderError} onCancel={()=>setConfirmDeleteFolder(null)} onDelete={removeFolder}/>}
    {routingManager&&<div className="label-modal-backdrop" onMouseDown={()=>{if(!ruleBusy)setRoutingManager(false)}}><section className="label-manager routing-manager" role="dialog" aria-modal="true" aria-labelledby="routing-manager-title" onMouseDown={event=>event.stopPropagation()}>
      <header><div><span className="label-manager-kicker">GMAIL / AUTOMATION</span><h2 id="routing-manager-title">Automation rules</h2><p>Route messages and assign attention levels automatically using exact sender addresses.</p></div><button className="label-manager-close" aria-label="Close automation rules" disabled={ruleBusy} onClick={()=>setRoutingManager(false)}><X size={18}/></button></header>
      ...
      <section className="priority-routing"><header><div><Flame size={17}/><span><b>Priority assignment rules</b><small>New and existing messages from the sender inherit this level.</small></span></div><i>{Object.keys(senderPriorityRules).length}</i></header><form onSubmit={event=>{event.preventDefault();createSenderPriorityRule()}}><input type="email" aria-label="Priority rule sender email address" value={priorityRuleEmail} onChange={event=>setPriorityRuleEmail(event.target.value)} placeholder="sender@company.com"/><div className="priority-rule-levels" aria-label="Priority level">{PRIORITY_LEVELS.map(level=>{const meta=PRIORITY_META[level],checked=priorityRuleLevel===level;return <button type="button" key={level} className={`priority-${level}${checked?' checked':''}`} style={{'--priority-color':meta.color} as CSSProperties} aria-pressed={checked} onClick={()=>setPriorityRuleLevel(level)}>{checked?<Check size={11}/>:<i/>}{meta.label}</button>})}</div><button disabled={ruleBusy||!priorityRuleEmail.trim()}><Plus size={14}/>{ruleBusy?'Saving…':'Save rule'}</button></form>{priorityRuleError&&<div className="routing-rule-error">{priorityRuleError}</div>}<PriorityRuleList rules={senderPriorityRules} busy={ruleBusy} onRemove={removeSenderPriorityRule}/></section>
    </section></div>}
